...
class PaymentController extends Controller
{
    ...
    public static function addFundRequest(Request $request)
    {
        ...
        $basic = (object)config('basic');
         if ($request->gateway == 'wallet') {
            ...
            if (!$gate) {
                return response()->json(['error' => 'Geçersiz Ödeme Ağ Geçidi'], 422);
            }


            if ($gate->min_amount > $reqAmount || $gate->max_amount < $reqAmount) {
                return response()->json(['error' => 'Lütfen İşlem Limitini Takip Edin'], 422);
            }
    ...
    public function depositConfirm(Request $request, $transaction)
    {
        ...
        if (is_null($order)) {
            return redirect()->route('user.addFund')->with('error', 'Geçersiz Fon Talebi');
        }
        if ($order->status != 0) {
            return redirect()->route('user.addFund')->with('error', 'Geçersiz Fon Talebi');
        }
    ...
    public function fromSubmit(Request $request)
    {
                        ...
                        if ($inVal->type == 'file') {
                            if ($request->hasFile($inKey)) {
                                try {
                                    ...
                                    session()->flash('error', 'Dosyanız yüklenemedi ' . $inKey);
        ...
        session()->flash('success', 'Talebiniz alındı.');
